...
chrome.runtime.onInstalled.addListener(function(details) {
  if (details.reason === 'install') {
    chrome.storage.sync.set( {
      "github-username" : "", 
      "github-repo-name" : "LeetCode-Problems-Solution-Python", 
      "github-token" : "" , 
...
chrome.webRequest.onCompleted.addListener(
  ...
  async function (details) {
    ...
    .then(data => {
      ...
      // console.log("Question nsjkhdhfkdshkf :", data.data.question);
    ...
    const questionURL = `https://leetcode.com/problems/${leetcode_URL_problemName}/`; // Construct problem URL
    console.log("Question URL print     :", questionURL);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch question page: ${response.status}`);
      }
      const htmlText = await response.text();
      console.log("HTML text:", htmlText);
      // const match = htmlText.match(/<meta name="description" content="(.*?)">/);
      const match = htmlText.match(/<meta\s+name="description"\s+content="([^"]+)"/);

      console.log("Match:skhdjfhsdhfhsdhakhkdsf dsaifukhadskuhfkd khadlflasdjfk ", match);
      if (match && match[1]) {
      ...
      console.error("Error fetching or parsing question content:", error);
    }
    ...
    // New logic to create and upload question.md
    const questionMarkdown = `# ${leetcode_questionTitle}\n\n${data.question_body}`;
    const questionFilePath = encodeURIComponent(`${questionId}. ${leetcode_questionTitle}/question.md`);
    const questionMessage = `Add question for ${leetcode_questionTitle}`;
    
    // Create the question.md file
    await fetch(`https://api.github.com/repos/${github_username}/${github_repo}/contents/${questionFilePath}`, {
      method: 'PUT',
      headers: {
      ...
      body: JSON.stringify({
        ...
        content: btoa(questionContent),
      }),
    });

    console.log('question.md created successfully.');
